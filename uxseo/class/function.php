...
// CSSインライン化
//-------------------
function css_inline($http, $gzip_article_html) {
...
//HTML圧縮
//-----------
function html_comp($gzip_article_html) {
	// HTML圧縮
	$search = array(
...
// ディレクトリ作成
//-------------------
function dir_create($directory_path) {
...
// マルチディレクトリ作成
//-------------------------
function multiple_directory_create($path, $directory_explode) {
	// 絶対パス
	$directory_path = $path;
...
//-------------------------------------
function html_gzip_create($gzip_article_html, $directory_path) {
	// articleファイル生成
